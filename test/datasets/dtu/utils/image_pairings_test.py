...
def test_image_pairings_valid_file(dtu_config_path):
    ...
    image_pairings = load_image_pairings(test_file_name, source_view_sampler=source_view_sampler)

    # check a few examples from file
    assert image_pairings.fetch(0).fetch() == [10, 1, 9]

    assert image_pairings.fetch(1).fetch() == [9, 10, 2]
    assert image_pairings.fetch(5).fetch() == [6, 18, 4]
