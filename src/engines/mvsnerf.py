...
from models.renderer_new import Renderer
from models.renderer_theirs import Renderer_ours
...
class MVSNeRF(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        image_features = self.feature_extraction(mvs_images[:, :-1])
        ...
        #leg_vol_features, _, _ = LegacyMVSNet()(mvs_images[:, 1:], batch['image_warp_matrices'][:, 1:], batch['depth_bounds'][0, 0], image_features, pad=self.hparams.image_feature_padding)
        #assert torch.allclose(volume_features, leg_vol_features, rtol=1e-2, atol=1e-5)
        ...
        peak_signal_to_noise = self.peak_signal_to_noise(prediction_rgb, all_image_colours[:, 0])

        with torch.no_grad():
            self.log('train/loss', mean_squared_error, prog_bar=True)
            self.log('train/mean_squared_error', mean_squared_error, prog_bar=True)
            self.log('train/peak_signal_to_noise', peak_signal_to_noise, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            volume_features = build_volume_features(
                image_features,
                # no need for the target image
                mvs_images[:, :-1],
                # no need for the target matrix
                batch['image_warp_matrices'][:, :-1],
                # use depth bounds for reference frustrum, which we map back to source views
                batch['depth_bounds'][:, 0].squeeze(1),
                padding=self.hparams.image_feature_padding,
            ...
            mean_squared_error = self.loss(image_prediction, mvs_images_target)
            peak_signal_to_noise = self.peak_signal_to_noise(image_prediction, mvs_images_target)
            ...
            # write images
            real = mvs_images_target[0].permute(1, 2, 0).cpu().numpy() * 255
            pred = image_prediction[0].permute(1, 2, 0).cpu().numpy() * 255
            out = numpy.concatenate(
                (real.astype(numpy.uint8), pred.astype(numpy.uint8)),
                axis=1
            )

            cv2.imwrite(f'.experiments/tmp/{self.global_step:08d}_{batch_idx:02d}_comparison.png', out)
            # mvs_images.shape: (batch_size, viewpoints, channels, height, width)
